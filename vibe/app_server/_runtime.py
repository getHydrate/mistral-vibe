...
class AgentRuntimeFactory:
    ...
    async def _hydrate_resumed(
        ...
        *,
        ...
        try:
            ...
            # Hydrate resume: arm the next session_start to fire with
            # source="resume" (the CLI _resume_previous_session wiring anchor
            # moved here when upstream lifted resume into the app-server).
            replacement.arm_session_start("resume")
    ...
    async def fork(self, source: AgentLoop, message_id: str | None) -> AgentLoop:
        ...
        # Hydrate fork: arm the next session_start to fire with source="fork"
        # (the AgentLoop.fork() wiring anchor moved here when upstream lifted
        # forking into the app-server runtime).
