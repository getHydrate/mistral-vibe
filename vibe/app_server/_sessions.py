...
from vibe.core.hooks.models import HookContextInjection
...
async def _drain_hook_events(events: AsyncGenerator) -> None:
    """Consume a hook-runner generator for its side effects. The subagent
    runner has no way to surface hook UI events mid-run, so they are drained
    and discarded.
    """
...
class SessionRuntimeRegistry(SubagentRunnerPort):
    ...
    async def run(
        ...
        # Parent-side subagent_start: an allowing hook's additional_context
        # is injected into the child conversation before its first turn runs.
        await self._fire_subagent_start(ctx, child, args)
        ...
        # Reported to the parent-side subagent_stop hook in the finally:
        ...
        try:
            try:
                ...
                if runtime.turns.active_turn is not None:
                    runtime.turns.interrupt(
                        TurnInterruptParams(
                            session_id=child.session_id,
                            expected_turn_id=response.turn.id,
                        )
                    )
            ...
            await self._fire_subagent_stop(
                ctx, child, agent_type=args.agent, status=status
            )

    async def _fire_subagent_start(
        self, ctx: InvokeContext, child: AgentLoop, args: TaskArgs
    ) -> None:
        """Parent-side subagent_start observation. An allowing hook's
        additional_context is injected into the child conversation before it
        runs (the child fires its own session events).
        """
    ...
    async def _fire_subagent_stop(
        ...
        *,
        ...
        """Parent-side subagent_stop observation — fires on success, failure,
        and cancellation alike. Shielded so a cancellation arriving during
        teardown cannot kill the audit; hook failures never mask the outcome.
        """
        if ctx.run_subagent_stop_hooks is None:
            return
        turn_count = sum(m.role is Role.assistant for m in child.messages)
        child_transcript_path = ""
        session_logger = child.session_logger
