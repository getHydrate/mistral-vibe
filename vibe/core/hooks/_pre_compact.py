...
class PreCompactHandler(HookHandler):
    """Runs before context compaction.

    Observational: compaction cannot be blocked. allow → inject
    ``hook_specific_output.additional_context``; the agent loop queues it
    so it survives the compaction reset and reappears on the next turn.
    ...
    def _on_deny(
        ...
        logger.warning(
            "Hook %s: pre_compact cannot be denied; ignoring deny", hook.name
        )
        return _HookAction(
            events=[
                HookEndEvent(
                    ...
                    content="deny ignored (compaction cannot be blocked)",
                )
            ],
