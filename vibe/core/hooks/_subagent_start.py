...
class SubagentStartHandler(HookHandler):
    """Runs in the PARENT session right before a task-tool subagent runs.

    Observational: a subagent start cannot be blocked. allow → inject
    ``hook_specific_output.additional_context`` into the CHILD conversation
    as a user message, if any. deny is logged and ignored.
    """
    ...
    def _on_deny(
        ...
        logger.warning(
            "Hook %s: subagent_start cannot be denied; ignoring deny", hook.name
        )
        return _HookAction(
            events=[
                HookEndEvent(
                    ...
                    content="deny ignored (subagent_start cannot be blocked)",
                )
            ],
