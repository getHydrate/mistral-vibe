...
| `post_agent` | Inject `reason` as a retry user message. Capped at 8 retries per hook per user turn; the invocation's `stop_hook_active` is `true` on retries caused by this hook's own previous deny. |
